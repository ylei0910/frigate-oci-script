...
if [ "$NON_INTERACTIVE" = true ]; then
    ...
    # Container ID
    read -p "Enter Container ID [100-999999999] (default: ${CLI_CT_ID:-$NEXT_CTID}): " CT_ID
...
if [[ ! "$CT_ID" =~ ^[0-9]+$ ]] || [ "$CT_ID" -lt 100 ] || [ "$CT_ID" -gt 999999999 ]; then
    error_exit "Invalid Container ID. Must be between 100 and 999999999."
fi
...
# The host media mount path is not auto-created by this installer - it must already exist.
if [ ! -d "$HOST_MEDIA_PATH" ]; then
    error_exit "Host Media Path '$HOST_MEDIA_PATH' does not exist. Create it (or mount your NAS share) before running this installer."
fi
...
# 6. Configure LXC container parameters
...
if [ "$DRY_RUN" = true ]; then
    ...
    # /config is not host-mounted - it stays inside the container's own rootfs,
    # so it's backed up/snapshotted along with the rest of the container.
    echo "" >> "$LXC_CONF"
    echo "# Frigate Host Bind Mounts (LXC native layout to support snapshots)" >> "$LXC_CONF"

    # Media uses a native Proxmox mp0 mount point, so it shows up in the GUI and
    # is recognized as shared/non-replicated NAS storage - appropriate for HA
    # cluster nodes where media typically lives on shared network storage.
    echo "mp0: $HOST_MEDIA_PATH,mp=/media,replicate=0,shared=1" >> "$LXC_CONF"
...
if [ "$DRY_RUN" = true ]; then
    ...
    if [ "$S6_READY" = true ]; then
        ...
        # /config is not part of the base OCI image and is no longer host-mounted,
        # so nothing else creates it - without this, Frigate crash-loops on first
        # boot trying to write config.yaml/frigate.db into a nonexistent directory.
        pct exec "$CT_ID" -- mkdir -p /config || log_warn "Failed to create /config inside container - Frigate will crash-loop until this exists."
...
**File Locations**
- Configuration: /config/config.yml (inside container)
...
echo "Config Path:    /config/config.yml (inside container)"
